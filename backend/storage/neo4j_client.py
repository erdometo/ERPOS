...
class JSONGraphAdapter:
    ...
    def create_or_update_node(self, node_id, label, name, description, skill_markdown, properties, clearance_level=1):
        ...
        if existing_node:
            ...
            existing_node["clearance_level"] = clearance_level
    ...
    def traverse_graph(self, start_node_name: str, role: str = None) -> list:
        ...
        start_node = None
        for n in nodes:
            if n["name"] == start_node_name and n.get("clearance_level", 1) <= clearance:
                start_node = n
                break
        ...
        for e in edges:
            ...
            if target_id is not None:
                t_node = None
                for n in nodes:
                    if n["id"] == target_id and n.get("clearance_level", 1) <= clearance:
                        t_node = n
                        break
                if t_node:
                    results.append({
    ...
    def get_all_nodes_with_edges(self, role: str = None) -> list:
        ...
        nodes_list = []
        for n in nodes:
            if n.get("clearance_level", 1) > clearance:
                continue
            nid = n["id"]
            node_edges = []
            for e in edges:
                if e["source_id"] == nid:
                    t_name = None
                    for tn in nodes:
                        if tn["id"] == e["target_id"] and tn.get("clearance_level", 1) <= clearance:
                            t_name = tn["name"]
                            break
                    if t_name:
                        node_edges.append({
